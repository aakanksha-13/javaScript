...
// Function to print details of pokemon whose name is passed as an argument
function myPokemonDetails(n) {
   var l=pokemonGo.pokemon.length;
   for(var i=0;i<l;i++)
    {
      if(pokemonGo.pokemon[i].name==n)
      {
        alert("Pokemon Details are:");
        alert(JSON.stringify(pokemonGo.pokemon[i]));// stringify i used to convert [object object] into string
        break;
      }
    }
  if(i==l){
...
// function to print name of all those pokemons which consist the same name in their next_evolution field as the name passed as an argument
function nextEvolutionName(n){
  var flag = 0;
  var i,j =" ";
  for( i in pokemonGo.pokemon)
  {
    for( j in pokemonGo.pokemon[i].next_evolution )
    {
      if((pokemonGo.pokemon[i].next_evolution[j].name)==n)
      {
        alert("Pokemon names with this name in their next_evolution field");
        alert(pokemonGo.pokemon[i].name);
        flag =1;
      }
    }
  }
  if(flag == 0)
  {
    alert("No Pokemon name corresponding to that name in next_evolution");
  }
}

// function to print name of all those pokemon's which have the weakness same as the passed arugment weakness
function pokemonWithWeakness(w) {
    var flag = 0;
    var i, j=" ";
    alert("Pokemon name's with that name in their next_evolution field: ");
  for(i in pokemonGo.pokemon)
  {
    for(j in pokemonGo.pokemon[i].weaknesses)
    {
      if(pokemonGo.pokemon[i].weaknesses[j]==w)
      {
        alert(pokemonGo.pokemon[i].name);
        flag=1;
      }
  ...
  if(flag == 0){
    alert("No Pokemon with this weakness");
  } 
}


alert("*********************MENU***********************");
alert("1. Take pokemon’s name as argument and display the information of that pokemon" + "\n" + 
"2.Take pokemon’s name as an argument and find out which all pokemon have that name in their next_evolution field." + "\n" +
 "3.Take a “Weakness” as an input and gives the names of all pokemon who have that Weakness.");
  
  var goOn="Yes";
  while(goOn=="Yes")
  {
    var choice=window.prompt("Enter your choice(1-3):");
    switch(choice)
    {
      case '1':  var pokemonName= window.prompt("Enter the pokemon name(First letter must be Capital):");
           myPokemonDetails(pokemonName);
           break;

      case '2':  var pokemonName=window.prompt("Enter the pokemon name(First letter must be Capital):");
            nextEvolutionName(pokemonName);
            break;

      case '3':  var weakness =window.prompt("Enter the weakness(First letter must be Capital):");
            pokemonWithWeakness(weakness);
            break;
    ...
    goOn=window.prompt("Do you want to continue(Yes/No)");
  }
alert(" You have made  Successfull exit!!")